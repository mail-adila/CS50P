...
# prompting them for items, one per line, until the user inputs control-d (which is a common way of
# ending one’s input to a program). After each inputted item, display the total cost of all items
...
def get_cost(food):
    food_item = food.strip().lower().title()
    return menu.get(food_item, 0)
